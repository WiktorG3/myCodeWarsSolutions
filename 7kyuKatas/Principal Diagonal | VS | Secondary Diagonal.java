...
public class Solution{
  public static String diagonal(int[][] matrix){
    ...
    for(int i = 0; i < matrix.length; i++){
      diagonalFirst += matrix[i][i];
      diagonalSecond+= matrix[i][j];
      j--;
    }
    return (diagonalFirst == diagonalSecond) ? "Draw!" : ((diagonalFirst > diagonalSecond) ? "Principal Diagonal win!" : "Secondary Diagonal win!");
  }
}
