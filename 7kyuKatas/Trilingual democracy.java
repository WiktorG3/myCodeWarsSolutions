...
public class TrilingualDemocracy {
  ...
  public static char trilingualDemocracy(char[] group) {
    ...
    for(char lang : group) {
      frequency.put(lang, frequency.getOrDefault(lang, 0) + 1);
    }
    ...
    if(frequency.size() == 2) {
      for (char lang : frequency.keySet()) {
        if(frequency.get(lang) == 1){
          return lang;
        }
      }
    }
    
    for(char lang : new char[] {'D', 'F', 'I', 'K'}) {
      if(!frequency.containsKey(lang)){
        return lang;
      }
    }
    return 'D';
